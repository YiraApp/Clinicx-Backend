...
// Create standard T-SQL script using GO as batch separators
const sqlBatches = [
...
async function main() {
    // Generate full SSMS script with GO separators
    const fullSql = `-- ====================================================================================
...
` + sqlBatches.join(";\nGO\n\n") + ";\nGO\n\nPRINT '✅ Production Migration Completed Successfully!';\nGO\n";
    ...
    fs.writeFileSync("scratch/production_migration_script.sql", fullSql);
    console.log("Wrote full script to scratch/production_migration_script.sql");

    // Test each batch against Production inside a transaction with rollback
    console.log("\nTesting all batches on Production DB inside a rollback transaction...");
    ...
    const queryRunner = prodDs.createQueryRunner();
    ...
    try {
        ...
        for (const batch of sqlBatches) {
            console.log(`Executing batch ${i}/${sqlBatches.length}...`);
            await queryRunner.query(batch);
            i++;
        }
        console.log("\n🎉 ALL 16 BATCHES EXECUTED WITH ZERO ERRORS ON PRODUCTION SQL SERVER!");
    } catch (err: any) {
        console.error("❌ Batch execution error:", err.message);
    } finally {
        console.log("Rolling back test transaction (no permanent changes made yet)...");
