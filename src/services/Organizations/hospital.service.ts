...
export class HospitalService implements IHospitalService {
    ...
    async getAllHospitals(orgId?: number, page: number = 1, pageSize: number = 10, grouped: boolean = false): Promise<any> {
        const { data: hospitals, total, stats } = await hospitalRepository.getAllHospitals(orgId, page, pageSize);
